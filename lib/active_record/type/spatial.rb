module ActiveRecord
  module Type
    class Spatial < Value # :nodoc:
      ...
      def set_geo_params(table_name, geometric_type)
      ...
      def cast_value(value)
        ...
        when ::RGeo::Feature::Geometry
          ...
          factory = RGeo::ActiveRecord::SpatialFactoryStore.instance.default(srid: value.srid)

          ::RGeo::Feature.cast(value, factory) rescue nil
        when ::String
          marker = value[4,1]
          if marker == "\x00" || marker == "\x01"
            # factory = @factory_settings.get_column_factory(@table_name, @column, :srid => value[0,4].unpack(marker == "\x01" ? 'V' : 'N').first)
            srid = value[0, 4].unpack(marker == "\x01" ? 'V' : 'N').first
            factory = RGeo::ActiveRecord::SpatialFactoryStore.instance.default(srid: srid)
